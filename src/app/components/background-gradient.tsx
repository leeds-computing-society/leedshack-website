...
export const BackgroundGradientMaterial = shaderMaterial(
  initialUniforms,
  vertexShader,
  fragmentShader,
);
...
export const BackgroundGradient = () => {
  ...
  let [targetColour1, setTargetColour1] = useState<Vector3>(
    new Vector3(0.0, 0.0, 0.0),
  );
  let [targetColour2, setTargetColour2] = useState<Vector3>(
    new Vector3(0.13725490196078433, 0.08235294117647059, 0.25098039215686274),
  );
  ...
  useFrame(({ clock }) => {
    ...
    if (
      gradientShader.current.colour1 === undefined ||
      gradientShader.current.colour2 === undefined
    )
      return;
    let new1X = calculateNewValue(
      targetColour1.x,
      gradientShader.current.colour1.x,
    );
    let new1Y = calculateNewValue(
      targetColour1.y,
      gradientShader.current.colour1.y,
    );
    let new1Z = calculateNewValue(
      targetColour1.z,
      gradientShader.current.colour1.z,
    );
    gradientShader.current.colour1 = new Vector3(new1X, new1Y, new1Z);
    let new2X = calculateNewValue(
      targetColour2.x,
      gradientShader.current.colour2.x,
    );
    let new2Y = calculateNewValue(
      targetColour2.y,
      gradientShader.current.colour2.y,
    );
    let new2Z = calculateNewValue(
      targetColour2.z,
      gradientShader.current.colour2.z,
    );
  ...
  useEffect(() => {
    if (pathname === "/2025") {
      setTargetColour1(
        new Vector3(0.0, 0.027450980392156862, 0.30196078431372547),
      );
      setTargetColour2(
        new Vector3(0.0, 0.5686274509803921, 0.3686274509803922),
      );
    }
    ...
    if (pathname === "/" || pathname === "/faq" || pathname === "/committee") {
      setTargetColour1(new Vector3(0, 0, 0));
      setTargetColour2(new Vector3(0.752, 0.1647, 0.988));
    }
  }, [pathname]);
